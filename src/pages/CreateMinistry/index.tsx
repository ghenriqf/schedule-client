      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s} className="flex items-center gap-2">
            <div className={`flex items-center gap-1.5 text-xs font-medium transition-colors ${active ? 'text-violet-600' : done ? 'text-emerald-500' : 'text-slate-300'}`}>
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] border transition-all ${active ? 'bg-violet-600 border-violet-600 text-white' : done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-200 text-slate-300'}`}>
                {done ? <IconCheck /> : i + 1}
              </div>
              <span className="hidden sm:inline">{s}</span>
            </div>
            {i < steps.length - 1 && (
              <div className={`w-8 h-px transition-colors ${done ? 'bg-emerald-300' : 'bg-slate-200'}`} />
            )}
          </div>
        )
      })}
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-7">
          ...
          <p className="text-sm text-slate-400 mt-1">
            {ctx.step === 0 && 'Dê um nome e descreva o propósito do ministério.'}
            {ctx.step === 1 && 'Adicione uma imagem para identificar o ministério.'}
            {ctx.step === 2 && 'Confira as informações antes de salvar.'}
          </p>
        </div>

        <form onSubmit={(e) => { e.preventDefault(); if (ctx.canSubmit) ctx.mutate() }}>
          ...
          {ctx.step === 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8 space-y-6">
              ...
              <Field label="Descrição" hint="Explique brevemente o propósito e atividades do ministério. (opcional)">
                <textarea
                  rows={4}
              ...
              <div className="flex justify-end pt-2">
                <button type="button" onClick={() => ctx.setStep(1)} disabled={!ctx.canGoNext}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8 space-y-6">
              <div className="flex flex-col items-center gap-4">
                <div className="relative group">
                  <div className="w-36 h-36 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center transition-all group-hover:border-violet-300">
                    {ctx.avatarPreviewUrl
                      ? <img src={ctx.avatarPreviewUrl} alt="Preview do ministério" className="w-full h-full object-cover" />
                      : <div className="flex flex-col items-center gap-2 text-slate-300"><IconCamera /><span className="text-xs">Sem foto</span></div>}
                  </div>
                  {ctx.avatarPreviewUrl && (
                    <button type="button" onClick={() => ctx.setAvatarImage(null)}
                ...
                <label className="cursor-pointer">
                  <input type="file" accept="image/*" className="sr-only" onChange={(e) => ctx.setAvatarImage(e.target.files?.[0] ?? null)} />
                ...
                <p className="text-xs text-slate-400 text-center max-w-xs">
                  Formatos PNG ou JPG. Se não enviar, o ministério será criado sem imagem — você pode adicionar depois.
                </p>
              </div>
              <div className="flex justify-between pt-2">
                <button type="button" onClick={() => ctx.setStep(0)} className="px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">← Voltar</button>
                <button type="button" onClick={() => ctx.setStep(2)} className="px-6 py-2.5 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 transition-colors">Continuar →</button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="h-32 bg-linear-to-br from-violet-100 via-indigo-50 to-blue-100 flex items-center justify-center overflow-hidden">
                  {ctx.avatarPreviewUrl
                    ? <img src={ctx.avatarPreviewUrl} alt="Preview" className="w-full h-full object-cover" />
                    : <span className="text-5xl opacity-25">🎸</span>}
                </div>
                <div className="p-6 space-y-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Nome</p>
                    <p className="text-base font-bold text-slate-800">{ctx.name}</p>
                  </div>
                  {ctx.description.trim() && (
                    <div>
                      <p className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Descrição</p>
                      <p className="text-sm text-slate-600 leading-relaxed">{ctx.description}</p>
                    </div>
                  )}
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-0.5">Foto</p>
                    <p className="text-sm text-slate-600">{ctx.avatarImage ? ctx.avatarImage.name : 'Nenhuma foto enviada'}</p>
                  </div>
                </div>
              </div>
              <div className="flex justify-between">
                <button type="button" onClick={() => ctx.setStep(1)} disabled={ctx.isPending}
                ...
                <button type="submit" disabled={!ctx.canSubmit}
                  ...
                  {ctx.isPending ? <><IconSpinner />Salvando…</> : <><IconCheck />Salvar ministério</>}
                </button>
              </div>
            </div>
          )}
