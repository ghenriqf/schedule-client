...
export function ScaleDetails() {
  ...
  const formattedDate = scale.scale
    ? format(new Date(scale.scale.date), "dd 'de' MMMM 'de' yyyy 'às' HH:mm", {
        locale: ptBR,
      })
    : "";

  const canAddMusic =
    !scale.isError && !!scale.scale && (scale.isAdmin || scale.isMinister);
  ...
  return (
    <>
        ...
        <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-slate-100 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {scale.isAdmin && !scale.isError && scale.scale && (
                <>
                  <button
                    onClick={() =>
                      navigate(
                        `/ministries/${scale.parsedMinistryId}/scales/${scale.parsedScaleId}/edit`,
                      )
                    }
                  ...
                  <button
                    onClick={() => scale.setMemberModalOpen(true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-200 text-slate-600 text-xs font-semibold rounded-xl hover:bg-slate-50 transition-colors"
                  >
                    + Membro
                  </button>
              ...
              {canAddMusic && (
                <button
                  onClick={() => scale.setMusicModalOpen(true)}
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-5">
                  ...
                  <div className="flex gap-3 shrink-0">
                    <Stat
                      icon="👥"
                      label="Membros"
                      value={scale.scale.members.length}
                    />
                    <Stat
                      icon="🎵"
                      label="Músicas"
                      value={scale.scale.musics.length}
                    />
                  </div>
                </div>
              </div>
                  ...
                  <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        {scale.scale.members.length} participantes
                      </p>
                    </div>
                    {scale.isAdmin && (
                      <button
                    ...
                    {scale.scale.members.length === 0 ? (
                      ...
                      scale.scale.members.map((member: ScaleMemberResponse) => (
                        <MemberRow
                          key={member.id}
                          member={member}
                          isAdmin={scale.isAdmin}
                          isRemoving={scale.isRemovingMember}
                          onRemove={scale.removeMember}
                        />
                      ))
                    )}
                  </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        {scale.scale.musics.length} músicas
                      </p>
                    </div>
                    {canAddMusic && (
                      <button
                      ...
                      <div className="space-y-1">
                        {scale.scale.musics.map((music: MusicResponse) => (
                          <MusicRow
                            ...
                            music={music}
                            isAdmin={scale.isAdmin}
                            isRemoving={scale.isRemovingMusic}
                            onRemove={scale.removeMusic}
                          />
                        ))}
                      </div>
                    )}
      ...
      <AddMusicModal
        ministryId={scale.parsedMinistryId}
        scaleId={scale.parsedScaleId}
        open={scale.musicModalOpen}
        onClose={() => scale.setMusicModalOpen(false)}
      />

      <AddMemberModal
        ministryId={scale.parsedMinistryId}
        scaleId={scale.parsedScaleId}
        open={scale.memberModalOpen}
        onClose={() => scale.setMemberModalOpen(false)}
      />
    </>
  );
}
