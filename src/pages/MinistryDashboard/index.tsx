...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-32 text-center">
      ...
      <div>
        <p className="text-sm font-semibold text-slate-700">Erro ao carregar ministério</p>
        <p className="text-xs text-slate-400 mt-1">Verifique sua conexão e tente novamente.</p>
      </div>
      <button onClick={onRetry} className="flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-600 text-xs font-semibold rounded-xl hover:bg-slate-50 transition-colors">
        Tentar novamente
      </button>
    </div>
  )
      ...
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <button onClick={dashboard.navigateToMinistries} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 transition-colors">
              <IconArrowLeft />
          ...
          {dashboard.ministry && (
            <span className={`shrink-0 text-[10px] font-semibold px-2.5 py-1 rounded-full border ${dashboard.ministry.role === 'ADMIN' ? 'bg-violet-50 border-violet-200 text-violet-700' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr_300px] gap-5 items-start">
            ...
            <aside className="space-y-4">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                {dashboard.ministry.avatarUrl ? (
                  <img src={dashboard.ministry.avatarUrl} alt={dashboard.ministry.name} className="w-full aspect-video object-cover" />
                ) : (
                  <div className="w-full aspect-video bg-linear-to-br from-violet-100 via-indigo-50 to-blue-100 flex items-center justify-center text-5xl opacity-30">🎸</div>
                )}
                <div className="p-4 space-y-4">
                  <div className="grid grid-cols-3 gap-2">
                    <Stat label="Membros" value={dashboard.ministry.ministryStats.memberCount} icon="👥" />
                    <Stat label="Escalas" value={dashboard.ministry.ministryStats.upcomingScalesCount} icon="📅" />
                    <Stat label="Músicas" value={dashboard.ministry.ministryStats.repertoryCount} icon="🎵" />
                  </div>
                  {dashboard.ministry.role === 'ADMIN' && (
                    <InviteCodeSection initialCode={dashboard.ministry.inviteCode} />
                  )}
                </div>
              </div>

              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <button onClick={dashboard.navigateToRepertoire} className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-violet-50 transition-colors group">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="text-sm font-semibold text-slate-700 group-hover:text-violet-700 transition-colors">Repertório</p>
                      <p className="text-[11px] text-slate-400">{dashboard.ministry.ministryStats.repertoryCount} músicas cadastradas</p>
                    </div>
                  </div>
                  <span className="text-slate-300 group-hover:text-violet-400 transition-colors"><IconChevronRight /></span>
                </button>
              </div>
            </aside>
            ...
            <main className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">Próximos eventos agendados</p>
                </div>
                {dashboard.ministry.role === 'ADMIN' && (
                  <button onClick={dashboard.navigateToCreateScale} className="bg-violet-600 hover:bg-violet-700 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-colors">
                    + Criar escala
                  </button>
                )}
              </div>
                ...
                {dashboard.scales.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {dashboard.scales.map((scale) => (
                      <ScaleCard key={scale.id} {...scale} ministryId={dashboard.ministryId ?? undefined} />
                    ))}
                  </div>
                )}
              </div>
            ...
            <aside className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-100">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">{dashboard.members.length} participantes</p>
              </div>
